var QueryBuilder = function() {
	...
	var extractHavingParts = function(key,key_array) {
		var m;
	...
	var compileDelete = function(qb) {
		...
		var sql = 'DELETE' + buildFromClause(qb) + buildWhereClause(qb);
		return buildLimitClause(sql,qb.limitTo,qb.offsetVal);
	};
	...
	return {
		...
		_like: function(field, match, type, side, not) {
			...
			for(k in field) {
				var like_statement;
				var v = field[k];
				var k = protectIdentifiers(this,k.trim());
		...
		_min_max_avg_sum: function(select,alias,type) {
			...
			var sql = type + '(' + protectIdentifiers(this,select.trim()) + ') AS ' + alias;
		...
		_having: function(key, value, type, escape) {
				...
				if (key_is_object === false) {
					if (typeof key === 'string') {
						if (value === null) {
							...
							key = extractHavingParts(key,key_array);
						}
					}
						...
						for (var i in key) {
							if (typeof key[i] !== 'string') {
								...
								key_array = extractHavingParts(key[i],key_array);
							}
						}
						key = key_array;
					}
			...
			for (var k in key) {
				var v = key[k];
				var prefix = (this.havingArray.length == 0 ? '' : type);
				...
				this.havingArray.push(prefix + k + v);
			}
		...
		set: function(key, value, escape) {
			...
			for (var i in key) {
				...
				if (escape === false) {
					this.setArray[protectIdentifiers(this,i,escape)] = v;
				} else {
					this.setArray[protectIdentifiers(this,i,escape)] = qb_escape(this,v);
				}
			}
