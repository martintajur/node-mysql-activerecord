var QueryBuilder = function() {
	...
	var trackAliases = function(qb,table) {
		...
		if (table.indexOf(' ') !== -1) {
			...
			if(qb.aliasedTables.indexOf(table) == -1) {
				qb.aliasedTables.push(table);
			}
		}
	}
	...
	var protectIdentifiers = function(qb,item,protect_identifiers) {
		...
		if (item.indexOf('.') !== -1) {
			...
			if (qb.aliasedTables.indexOf(parts[0]) !== -1) {
	...
	return {
		...
		join: function(table, relation, direction) {
			...
			if (match = relation.match(/([\w\.]+)([\W\s]+)(.+)/)) {
				match[1] = protectIdentifiers(this,match[1]);
				match[3] = protectIdentifiers(this,match[3]);
				
				relation = match[1] + match[2] + match[3];
			}
			
			join = direction + 'JOIN ' + protectIdentifiers(this,table, true) + (relation !== '' ? ' ON ' + relation : ' ' + relation);
